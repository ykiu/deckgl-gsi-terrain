// Modified version of
// https://github.com/Kanahiro/deckgl-gsi-terrain-layer/blob/master/gsi-terrain-loader/parse-terrain.js
// https://github.com/visgl/loaders.gl/blob/3.0-release/modules/terrain/src/lib/parse-terrain.js
import { getMeshBoundingBox } from "@loaders.gl/schema";
import Delatin from "@loaders.gl/terrain/src/lib/delatin";
import { addSkirt } from "@loaders.gl/terrain/src/lib/helpers/skirt";
...
function getTerrain(imageData, width, height, elevationDecoder, tesselator) {
  ...
  for (let i = 0, y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1, i += 1) {
      ...
      terrain[i + y] =
        (r * rScaler + g * gScaler + b * bScaler + gsiOffset) * scaler + offset;
    }
  }

  if (tesselator === "martini") {
    // backfill bottom border
    for (let i = (width + 1) * width, x = 0; x < width; x += 1, i += 1) {
      terrain[i] = terrain[i - width - 1];
    }
    // backfill right border
    for (let i = height, y = 0; y < height + 1; y += 1, i += height + 1) {
...
function getMeshAttributes(vertices, terrain, width, height, bounds) {
  ...
  for (let i = 0; i < numOfVerticies; i += 1) {
